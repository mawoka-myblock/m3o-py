...
@pytest.mark.asyncio
async def test_get():
    assert await cache.get("test") == {"key": "test", "value": "100", "ttl": "3599"} or {"key": "test", "value": "100",
                                                                                         "ttl": "3598"}


@pytest.mark.asyncio
async def test_decrement():
    # assert await cache.decrement("test", 1) == {"key": "test", "value": 99}
    assert True
...
async def test_delete():
    assert await cache.delete("test") == {"status": "ok"}
